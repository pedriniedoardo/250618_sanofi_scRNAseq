# AIM ---------------------------------------------------------------------
# put together the dataset from shaflick CSF, shafflick blood and full brain dataset WM+CX to compare a panel fo genes for Michele's project
...
write_tsv(df_counts_long,"../out/table/015_df_counts_long_Michele.tsv")
...
ggsave("../out/plot/015_scatter_expression_normCount_Michele.pdf",width = 9,height = 6)
...
df_counts_long %>%
  ...
  theme(strip.background =element_blank(),
        axis.text.x = element_text(angle = 90,hjust = 1))
ggsave("../out/plot/015_scatter_expression_CPM_Michele.pdf",width = 9,height = 6)
...
ggsave("../out/plot/015_heatmap_expression_Michele.pdf",width = 8,height = 4)

# try tailor the plot to match the sytle requested
df_counts_long %>%
  # force the order
  mutate(sample = factor(sample,levels = c("brain_EPENDYMA","brain_EXC.NEU","brain_INH.NEU","brain_AST","brain_OPC","brain_OLIGO","brain_VAS","brain_LYM","brain_IMM",
                                           "csf_MONO","csf_B","csf_CD4.T","csf_CD4.T.clu3","csf_CD8.T.clu1","csf_CD8.T.clu4","csf_NK","csf_DC",
                                           "blood_MONO","blood_B","blood_CD4.T","blood_CD8.T","blood_NK","blood_DC","blood_other"
  ))) %>% 
  mutate(sample = fct_rev(sample)) %>%
  mutate(gene = fct_relevel(gene,c("SPP1","CHIT1","CHI3L1"))) %>%
  # mutate(cat = factor(cat,levels = c("ASTRO","B_CELLS","T_CELLS","ENDOTHELIAL","PERICYTE","IMMUNE","NEURONS","OLIGO","OPC"))) %>% 
  ...
  scale_fill_gradientn(colours = viridis::turbo(20),
                       ...
                       name = 'norm exp z-scaled\nwithin genes') +
  theme(plot.margin = margin(1.5, 0.5, 0.5, 0.5, "cm"))
ggsave("../out/plot/015_heatmap_expression_Michele02.pdf",width = 5,height = 8)
...
pdf("../out/plot/015_heatmap_expression_complexHeatmap_Michele.pdf",width = 8,height = 4)
...
# comapre without batch correction
vst_unfilter <- readRDS("../out/object/015_vst_unfiltered.rds")

# attempt PCA to see the effect fo the batch correction
plot_vst <- plotPCA(vst_unfilter,
                    intgroup = c("dataset","cell_id")) +
  theme_bw()

# extract the pca values
df_plot_PCA <- plot_vst$data %>%
  # harmonize the name of the cells
  mutate(cellid_harmonized = case_when(str_detect(cell_id,pattern = "CD8|CD4")~"T",
                                       str_detect(cell_id,pattern = "NEU")~"NEU",
                                       T ~ cell_id))
...
p1 <- df_plot_PCA %>%
  ...
  theme_bw() + 
  ylab(plot_vst$labels[1]) + xlab(plot_vst$labels[2]) + # Fixed labels
  custom_scale_color +
...
p2 <- df_plot_PCA %>%
  ...
  theme_bw() + 
  ylab(plot_vst$labels[1]) + xlab(plot_vst$labels[2]) + # Fixed labels
  ggtitle("Uncorrected: Origin")

# --- 2. BATCH CORRECTED PLOTS ---
p1_batch <- df_plot_PCA_batch %>%
  ggplot(aes(x=PC1, y=PC2, col=cellid_harmonized)) +
  geom_point(size =4, alpha=0.6) +
  scale_x_continuous(expand = expansion(mult = 0.1)) +
  theme_bw() + 
  ylab(plot_vst_batch$labels[1]) + xlab(plot_vst_batch$labels[2]) +
  custom_scale_color +
  ggtitle("Corrected: Cell Type")
...
p2_batch <- df_plot_PCA_batch %>%
  ...
  theme_bw() + 
  ylab(plot_vst_batch$labels[1]) + xlab(plot_vst_batch$labels[2]) +
  ggtitle("Corrected: Origin")

# --- 3. COMBINE EVERYTHING INTO ONE PLOT ---
...
# draw(ht2,heatmap_legend_side = "left")
pdf("../out/plot/015_heatmap_expression_complexHeatmap_batch_Michele.pdf",width = 8,height = 4)
...
# save the table for quicker exploration
write_tsv(vst_value,"../out/table/015_df_vst_long_Michele.tsv")
...
vst_value %>%
  ...
  theme(strip.background =element_blank(),
        axis.text.x = element_text(angle = 90,hjust = 1))
ggsave("../out/plot/015_scatter_expression_batch_Michele.pdf",width = 9,height = 6)
...
vst_value %>%
  ...
  theme(plot.margin = margin(1.5, 0.5, 0.5, 0.5, "cm"))
ggsave("../out/plot/015_heatmap_expression_batch_Michele.pdf",width = 8,height = 4)
...
vst_value %>%
  ...
  scale_fill_gradientn(colours = viridis::viridis(20),
                       ...
                       name = 'vst batch z-scaled\nwithin genes') +
  theme(plot.margin = margin(1.5, 0.5, 0.5, 0.5, "cm"))
ggsave("../out/plot/015_heatmap_expression_batch_Michele02.pdf",width = 5,height = 8)
